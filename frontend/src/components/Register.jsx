...
const Register = () => {
    ...
    async function handleSubmit(e) {
        ...
        if (isEmailValid && name.length != 0 && password.length != 0) {
            try {
        ...
        <div className='move-to-center'>
            ...
            <form onSubmit={handleSubmit}>
                <div>
                    ...
                    <input className='register-inputs' type="text" value={name} id='name' onChange={(e) => setName(e.target.value)} />
                </div>

                <div>
                    <label htmlFor="email">Email</label>
                    <input className='register-inputs' type="email" value={email} id='email' onChange={(e) => setEmail(e.target.value)} />
                </div>
